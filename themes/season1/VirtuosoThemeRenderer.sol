...
/// @title  Speedtracer Virtuoso Theme
/// @notice Colors, font and names for Virtuoso Club themed Speedtracer tracks
contract VirtuosoThemeRenderer {
    bytes16 private constant SYMBOLS = "0123456789ABCDEF";

    uint24 public constant BACKGROUND_COLOR = 0x030112;
    uint24 public constant KERB_COLOR = 0xEAFA52;
    uint24 public constant ROAD_COLOR = 0x1C0998;
    uint24 public constant MIDLINE_COLOR = 0xFFFFFF;

    // This is the Press Start 2P font from Google Fonts.
    // It's all of the glyphs, but we can take any font file and select just the
    // glyphs we need for the track name to save space and ensure we can fit it onchain.
    ...
    function themeName() external pure returns (string memory) {
        return "Virtuouso Club";
    }

    function name(uint256) external pure returns (string memory) {
        return "Kings Crossing";
    }

    // This is where we can do some fun stuff to put more things onchain!
    // Like if you have an SVG of your logo that is small enough, we can add that at the end of
    // this bg function so that it appears as part of the onchain SVG NFT.
    ...
    // This part is a little weird tbh. The TrackRenderer depends on there being some filters defined.
    // So this is a "no filter" filter for the road texture, and a filter that creates a little 3D effect for the track name.
