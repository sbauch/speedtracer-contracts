...
contract SaudiArabianGPThemeRenderer is IThemeRenderer {
    bytes16 private constant SYMBOLS = "0123456789ABCDEF";

    uint24 public constant BACKGROUND_COLOR = 0x011425;
    uint24 public constant KERB_COLOR = 0xFCD800;
    uint24 public constant ROAD_COLOR = 0xE2061F;
    uint24 public constant MIDLINE_COLOR = 0xFCD800;
    ...
    function background() external pure returns (string memory) {
        return
            "<rect id='bg' width='100%' height='100%' fill='url(#gradient)' />";
    }
    ...
    function filter(uint256) external pure returns (string memory) {
        return
        '<filter id="noise"><feComponentTransfer><feFuncR type="identity" /><feFuncG type="identity" /><feFuncB type="identity" /><feFuncA type="identity" /></feComponentTransfer></filter><filter id="tf" x="-20%" y="-20%" width="140%" height="140%"><feGaussianBlur stdDeviation="2 2" result="shadow"/><feOffset dx="2" dy="2"/></filter><radialGradient id="gradient" cx="0" cy="0" r="1" gradientUnits="userSpaceOnUse" gradientTransform="translate(985.5 1301) rotate(-141.248) scale(1263.68 777.65)"><stop stop-color="#011425" /><stop offset="1" stop-color="#001A30" /></radialGradient>';
    }
}
